...
def create_frame(parent):
    ...
    # --- Title -----------------------------
    title_label = tk.Label(frame, text="Self-Inductance of Single-Layer Ring Coil with circular cross-section (low. freq.)", font=("Arial", 16, "bold"), bg="white")
    ...
    # --- Entry Fields ---------------------
    labels = ["Winding diameter D₁", "Mean radius coil diameter D₂", "Number of turns w"]
    entries = []
    default_values = ["1","10","300"]
    ...
    def calculate():
        try:
            ...
            ind=2*np.pi* (w**2) * (D2 - np.sqrt(D2**2 - D1**2))
...
#    text.insert("1.0",quote)
    ...
    footer = tk.Label(
        frame,
        text=r"Harry Hertwig: Induktivitäten. Berlin: Verlag für Radio-Foto-Kinotechnik. 1954. Induktivität einer Ringspule mit kreisförmigen Querschnitt.",
        bg="white",
        font=("Arial", 10),
        fg="gray"
    )
