...
def create_frame(parent):
    ...
    # --- Title -----------------------------
    title_label = tk.Label(frame, text="Self-Inductance of a straight rectangular Rod (low freq.)", font=("Arial", 16, "bold"), bg="white")
    title_label.grid(row=0, column=0, columnspan=6, sticky="w", padx=10, pady=10)

    # --- Image (Top-Right) ----------------
    image_path = os.path.join(os.path.dirname(__file__), "pic_straight rectangular Rod.png")
    try:
    ...
    # --- Entry Fields ---------------------
    labels = ["Length l", "Width b", "Thickness c"]
    entries = []
    default_values = ["3","8","2.5"]
    ...
    def calculate():
        try:
            ...
            c = float(entries[2].get())*100*unit_factors_length[thickness_unit_var.get()] #m->cm

            inductance =  (2*l*(np.log((2*l)/(b+c))+0.5+0.2235* (b+c)/l)) * 10**(-9) * unit_factors_inductance[output_unit_var.get()]
...
#    text.insert("1.0",quote)
    ...
    footer = tk.Label(
        frame,
        text=r"Harry Hertwig: Induktivitäten. Berlin: Verlag für Radio-Foto-Kinotechnik. 1954. Induktivität eines geraden rechteckigen Stabes.",
        bg="white",
        font=("Arial", 10),
        fg="gray"
    )
