...
def create_frame(parent):
    ...
    title_label = tk.Label(
        frame,
        text="Self-Inductance of a concentric cable (low freq.)",
        font=("Arial", 16, "bold"),
        bg="white"
    )
    ...
    # --- Entry Fields ---------------------
    labels = ["Length l", "Inner diameter d", "Outer diameter D"]
    entries = []
    default_values = ["3", "2", "10"]

    # Unit selection variables
    length_unit_var = tk.StringVar(value="m")
    d_unit_var = tk.StringVar(value="mm")
    D_unit_var = tk.StringVar(value="mm")

    for i, label in enumerate(labels):
        lbl = tk.Label(frame, text=label, bg="white", anchor="w")
        ...
        ent.grid(row=i+2, column=1, padx=5, pady=5)
        entries.append(ent)

        # Unit dropdowns
        unit_cb = ttk.Combobox(frame, values=list(unit_factors_length.keys()), width=6, state="readonly")
        if i == 0:
            unit_cb.config(textvariable=length_unit_var)
        elif i == 1:
            unit_cb.config(textvariable=d_unit_var)
        elif i == 2:
            unit_cb.config(textvariable=D_unit_var)
        unit_cb.grid(row=i+2, column=2, padx=(2, 0), pady=5)

    # --- Result Output ---------------------
    result_label = tk.Label(frame, text="Inductance L", bg="white", anchor="w")
    result_label.grid(row=6, column=0, sticky="w", padx=10, pady=(15, 5))
    ...
    precision_label.grid(row=6, column=3, sticky="w", padx=10, pady=5)
    ...
    def calculate():
        try:
            ...
            # Apply unit conversions to meters
            l = l * unit_factors_length[length_unit_var.get()]
            d = d * unit_factors_length[d_unit_var.get()]
            D = D * unit_factors_length[D_unit_var.get()]
            ...
            inductance_H = (2*l*(np.log(D/d)+0.25)) * 10**(-9)

            # Convert to selected output unit
            converted_L = inductance_H * unit_factors_inductance[output_unit_var.get()]
    ...
    calc_button.grid(row=7, column=1, columnspan=1, pady=(10, 5))
